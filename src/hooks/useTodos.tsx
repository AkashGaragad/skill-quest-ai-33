...
export function useTodos() {
  ...
  const fetchTodos = async () => {
    ...
    try {
      ...
      const categorizedTodos = {
        thisWeek: data?.filter(todo => todo.status === 'thisWeek').map(todo => ({
          ...todo,
          priority: todo.priority as 'low' | 'medium' | 'high',
          status: todo.status as 'thisWeek' | 'inProgress' | 'completed',
          completed_at: todo.completed_at || undefined
        })) || [],
        inProgress: data?.filter(todo => todo.status === 'inProgress').map(todo => ({
          ...todo,
          priority: todo.priority as 'low' | 'medium' | 'high',
          status: todo.status as 'thisWeek' | 'inProgress' | 'completed',
          completed_at: todo.completed_at || undefined
        })) || [],
        completed: data?.filter(todo => todo.status === 'completed').map(todo => ({
          ...todo,
          priority: todo.priority as 'low' | 'medium' | 'high',
          status: todo.status as 'thisWeek' | 'inProgress' | 'completed',
          completed_at: todo.completed_at || undefined
  ...
  const toggleTodo = async (todoId: string, fromStatus: string) => {
    const todo = todos[fromStatus as keyof typeof todos]?.find(t => t.id === todoId);
    if (!todo) return;
