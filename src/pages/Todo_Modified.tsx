...
import { Plus, Calendar, Flag, CheckCircle, Circle, Flame, Code } from "lucide-react";
...
const Todo = () => {
  ...
  // Suggested tasks based on selected roadmap
  const getSuggestedTasks = () => {
    if (!selectedRoadmap) return [];
    
    if (selectedRoadmap.id === 'web-dev') {
      return [
        "Practice HTML semantic elements",
        "Build a responsive CSS grid layout",
        "Complete JavaScript array methods exercises",
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Header with Streak and Roadmap Info */}
          ...
          <div>
            ...
            {selectedRoadmap && (
              <div className="mt-3 flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span>Focus: {selectedRoadmap.title}</span>
              </div>
            )}
          </div>
        ...
        <div className="bg-gradient-card rounded-2xl p-6 shadow-soft border border-border mb-8 animate-slide-up">
          ...
          <div className="flex gap-4 mb-4">
            <input
              ...
              placeholder={selectedRoadmap ? `Add a task for ${selectedRoadmap.title}...` : "What do you want to learn today?"}
          ...
          {/* Suggested Tasks for Selected Roadmap */}
          {selectedRoadmap && getSuggestedTasks().length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-muted-foreground mb-3">Suggested tasks for {selectedRoadmap.title}:</h4>
              <div className="flex flex-wrap gap-2">
                {getSuggestedTasks().map((suggestion, index) => (
                  <button
                    key={index}
