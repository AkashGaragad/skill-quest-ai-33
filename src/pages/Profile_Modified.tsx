...
import { Edit3, MapPin, Calendar, Trophy, Target, BookOpen, Flame, Star, TrendingUp, Code } from "lucide-react";
...
const Profile = () => {
  ...
  // Prioritize selected roadmap in display
  const prioritizedRoadmaps = roadmaps ? [
    ...roadmaps.filter(r => selectedRoadmap && r.id === selectedRoadmap.id),
    ...roadmaps.filter(r => !selectedRoadmap || r.id !== selectedRoadmap.id)
  ] : [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center gap-6 mb-6 md:mb-0">
              ...
              <div>
                ...
                <p className="text-white/80 text-lg mb-2">
                  {selectedRoadmap ? `Focused on ${selectedRoadmap.title}` : "Aspiring Full-Stack Developer"}
                </p>
                ...
                {selectedRoadmap && (
                  <div className="mt-2 flex items-center gap-2 bg-white/10 rounded-lg px-3 py-1">
                    <Code className="w-4 h-4" />
                    <span className="text-sm">Current focus: {selectedRoadmap.title}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Learning Progress - Prioritize Selected Roadmap */}
              ...
              <h3 className="text-xl font-semibold text-foreground mb-6 flex items-center gap-2">
                ...
                {selectedRoadmap && (
                  <span className="text-sm text-muted-foreground">
                    (Focusing on {selectedRoadmap.title})
                  </span>
                )}
              </h3>
              <div className="space-y-4">
                {prioritizedRoadmaps.map((roadmap, index) => (
                  <div 
                    key={roadmap.id} 
                    className={`space-y-2 ${selectedRoadmap && roadmap.id === selectedRoadmap.id ? 'p-4 bg-primary/5 rounded-lg border border-primary/20' : ''}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-foreground">{roadmap.title}</span>
                        {selectedRoadmap && roadmap.id === selectedRoadmap.id && (
