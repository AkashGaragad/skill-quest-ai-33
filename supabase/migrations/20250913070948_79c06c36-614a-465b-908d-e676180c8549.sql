...
CREATE POLICY "Users can view their own user stats" ON public.user_stats FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own user stats" ON public.user_stats FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can modify their own user stats" ON public.user_stats FOR UPDATE USING (auth.uid() = user_id);
